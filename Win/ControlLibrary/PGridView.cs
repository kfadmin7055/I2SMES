...
namespace EBAP.Win.ControlLibrary
{
    ...
    /// 
    /// </summary>
    public partial class PGridView : DevExpress.XtraGrid.Views.Grid.GridView, IFillData, IInitColumn, IMandatoryColumns
    {
        ///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        /// GridView를 생성합니다.
        /// </summary>
        public PGridView(DevExpress.XtraGrid.GridControl ownerGrid)
            : base(ownerGrid)
        {
            InitializeComponent();
        }
        ...
        public void InitColumn(string fieldName, string caption, int width, int maxLength, int decimalPlace, bool allowEdit, bool visible, DataType dataType, HorzAlign horzAlign)
        {
            ...
            if (!existColumn) Columns.Add(gridColumn);
            ...
            gridColumn.AppearanceCell.TextOptions.HAlignment = AppUtil.StringToEnum<HorzAlignment>(horzAlign.ToString());
            //switch (horzAlign)
            //{
        ...
        /// 
        /// </summary>
        public void InitColumnFromDB()
        {
            DataSet ds = (GridControl.FindForm() as IGridColumnSet).GetGridColumnSet(Name);
            DataTable dt1 = ds.Tables[0];
            DataRow drDetail = null;
            DataType dataType = DataType.Default;
            HorzAlign horzAlign = HorzAlign.Default;

            if (ds.Tables[1].Rows.Count > 0)
            {
                drDetail = ds.Tables[1].Rows[0];

                if (Convert.ToBoolean(drDetail["USESELECTCOLUMN"]))
                    InitColumn(AppConfig.CHECKCOLUMNNAME, "Select", 50, 0, true, true, DataType.CheckEdit);
            }
            ...
            foreach (DataRow dr in dt1.Rows)
            {
                ...
                int decimalPlace = Convert.ToInt32(dr["DECIMALPLACE"]);
                bool allowEdit = Convert.ToBoolean(dr["ALLOWEDIT"]);
                bool visible = Convert.ToBoolean(dr["VISIBLE"]);

                string dType = dr["DATATYPE"].ToString();
                string hAlign = dr["HORIZONALIGN"].ToString();

                dataType = AppUtil.StringToEnum<DataType>(dType);
                horzAlign = AppUtil.StringToEnum<HorzAlign>(hAlign);

                InitColumn(fieldName, caption, width, maxlength, decimalPlace, allowEdit, visible, dataType, horzAlign);

                //if(decimalPlace == 0)
                //    InitColumn(fieldName, caption, width, maxlength, allowEdit, visible, dataType, horzAlign);
                //else
                //    InitColumn(fieldName, caption, width, maxlength, decimalPlace, allowEdit, visible, dataType, horzAlign);
            }
            ...
            string[] kColumns = null;
            string[] mColumns = null;
            string[] neColumns = null;
            string[] ncColumns = null;

            if (drDetail["KEYCOLUMNS"].ToString().Replace(" ", "") != string.Empty)
                kColumns = drDetail["KEYCOLUMNS"].ToString().Replace(" ", "").Split(',');

            if (drDetail["MANDATORYCOLUMNS"].ToString().Replace(" ", "") != string.Empty)
                mColumns = drDetail["MANDATORYCOLUMNS"].ToString().Replace(" ", "").Split(',');

            if (drDetail["NEWROWENABLECOLUMNS"].ToString().Replace(" ", "") != string.Empty)
                neColumns = drDetail["NEWROWENABLECOLUMNS"].ToString().Replace(" ", "").Split(',');

            if (drDetail["NEWROWCOPYCOLUMNS"].ToString().Replace(" ", "") != string.Empty)
                ncColumns = drDetail["NEWROWCOPYCOLUMNS"].ToString().Replace(" ", "").Split(',');

            KeyColumns = kColumns;
            MandatoryColumns = mColumns;
            NewRowEnableColumns = neColumns;
            NewRowCopyColumns = ncColumns;
        }
        ...
        ///// <param name="e"></param>
        //private void ButtonColumnEdit_ButtonEditClick(object sender, ButtonPressedEventArgs e)
        //{
        //    if (ButtonEditClick == null) return;
        //    CloseEditor(false);
        ...
        public void InitUnboundColumn(string fieldName, string caption, int width, string expression, UnboundType unboundType = UnboundType.Integer, HorzAlign horzAlign = HorzAlign.Far)
        {
            EBAP.Core.Localization.LocaleConverter locale = (GridControl.FindForm() as ILocaleConverter).LOCALECONVERTER;
            ...
            gridColumn.UnboundType = AppUtil.StringToEnum<UnboundColumnType>(unboundType.ToString());

            gridColumn.Visible = true;
            gridColumn.UnboundExpression = expression;
            gridColumn.ShowUnboundExpressionMenu = true;

            //ControlUtil.SetColumnType(gridColumn, DataType.Number, 0, 0);
            gridColumn.AppearanceHeader.Font = ControlConfig.UNDERLINEFONT;
            gridColumn.AppearanceCell.TextOptions.HAlignment = AppUtil.StringToEnum<HorzAlignment>(horzAlign.ToString());
        ...
        #region :: UseIndicatorNumber :: Row Indicator에 숫자를 표시합니다.

        private bool _useIndicatorNumber = false;
        /// <summary>
        /// Row Indicator에 숫자를 표시합니다.
        /// </summary>
        [Category("EBAP"), DefaultValue(false), Browsable(true)]
        [Description("Row Indicator에 숫자를 표시합니다.")]
        public bool UseIndicatorNumber
        {
            get
            {
                return _useIndicatorNumber;
            }
            set
            {
                _useIndicatorNumber = value;

                if (value) IndicatorWidth = 40;
                else IndicatorWidth = -1;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public void CheckMandatoryColumns()
        {
            if (MandatoryColumns == null || MandatoryColumns.Length == 0) return;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    if (dr[column] == null || dr[column].ToString() == string.Empty) 
        ...
        /// <summary>
        /// Check Edit Column을 초기화합니다.
        /// </summary>
        /// <param name="fieldName"></param>
        /// <param name="valueChecked"></param>
        /// <param name="valueUnchecked"></param>
        public void InitCheckEditColumn(string fieldName, object valueChecked, object valueUnchecked)
        {
            GridColumn gridColumn = Columns[fieldName];

            if (gridColumn == null) return;

            RepositoryItemCheckEdit edit = RepositoryItemEditor.CheckEdit();
            edit.ValueChecked = valueChecked;
            edit.ValueUnchecked = valueUnchecked;
        ...
        {
            return GetCheckedData(AppConfig.CHECKCOLUMNNAME, "1");
        }
        ...
        /// 
        /// </remarks>
        public DataTable GetCheckedData(string columnName, string isChecked)
        {
            CloseEditor(true);
            ...
            {
                if ((dr[columnName]?.ToString() == "1") == (isChecked == "1"))
                    dt.ImportRow(dr);
        ...
        public DataTable GetDataTableByDataSource()
        {
            ...
            if (GridControl.DataSource == null) return null;

            DataTable dt = (GridControl.DataSource as DataSet).Tables[GridControl.DataMember];
        ...
        #region :: GetFocusedRowComboColumnValue(+1 Overloading) :: Grid의 콤보박스에서 원하는 컬럼의 값을 가져옵니다.
        ...
        /// <returns></returns>
        public string GetFocusedRowComboBoxColumnValue(string columnName)
        {
            return GetFocusedRowComboBoxColumnValue(FocusedColumn.FieldName, GetFocusedRowCellValue(FocusedColumn), columnName);
        }
        ...
        /// <param name="columnName">가져올 컬럼명</param>
        /// <returns></returns>
        public string GetFocusedRowComboBoxColumnValue(string fieldName, object value, string columnName)
        {
            GridColumn gridColumn = Columns[fieldName];
            ...
            int idx = edit.GetDataSourceRowIndex(edit.ValueMember, value);
        ...
        public void RemoveCheckedRow(string fieldName)
        {
            ...
            {
                if (!ds.Tables[GridControl.DataMember].Rows[i][fieldName].Equals(true))
                    continue;
        ...
        public void SetColumnCharacterCasing(CharacterCasing cashing, params string[] fieldNames)
        {
            ...
            {
                GridColumn gridColumn = base.Columns[fieldName];
        ...
        #region :: SetColumnDataType :: Grid Column의 데이터 형식을 설정합니다.

        /// <summary>
        /// Grid Column의 데이터 형식을 설정합니다.
        /// </summary>
        /// <param name="fieldName">Column Field 명</param>
        ...
        /// 
        /// </remarks>
        public void SetColumnDataType(string fieldName, DataType dataType)
        {
            SetColumnDataType(fieldName, dataType, 0, 0);
        }
        ...
        /// <param name="fieldName">Column Field 명</param>
        /// <param name="dataType">데이터 형식</param>
        /// <param name="decimalPlace">변경할 자리수</param>
        /// <remarks>
        /// 2016-05-20 최초생성 : 오인봉
        /// 변경내역
        /// 
        /// </remarks>
        public void SetColumnDataType(string fieldName, DataType dataType, int decimalPlace)
        {
            SetColumnDataType(fieldName, dataType, 0, decimalPlace);
        }
        ...
        /// 
        /// </remarks>
        public void SetColumnDataType(string fieldName, DataType dataType, int maxLength, int decimalPlace)
        {
            GridColumn gridColumn = base.Columns[fieldName];
        ...
        public void SetColumnSummary(string fieldName, SummaryType type)
        {
            ...
            switch (type)
            {
                case SummaryType.Sum:
                    ...
                    string format = string.IsNullOrEmpty(gridColumn.DisplayFormat.FormatString)
                                    ? "n0"
                                    : gridColumn.DisplayFormat.FormatString;

                    gridColumn.SummaryItem.DisplayFormat = $"{{0:{format}}}";
                    break;
                case SummaryType.Average:
                    gridColumn.SummaryItem.SummaryType = SummaryItemType.Average;
                    gridColumn.SummaryItem.DisplayFormat = String.Format("Avg : {{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
        ...
        public void SetGroupSummary(string fieldName, SummaryType type)
        {
            ...
            switch (type)
            {
                case SummaryType.Sum:
                    ...
                    dispFormat = String.Format("{{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
                case SummaryType.Average:
                    itemType = SummaryItemType.Average;
                    dispFormat = String.Format("Avg : {{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
                case SummaryType.Count:
                    itemType = SummaryItemType.Count;
                    dispFormat = String.Format("Cnt : {{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
                case SummaryType.Max:
                    itemType = SummaryItemType.Max;
                    dispFormat = String.Format("Max : {{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
                case SummaryType.Min:
                    itemType = SummaryItemType.Min;
                    dispFormat = String.Format("Min : {{0:{0}}}", gridColumn.ColumnEdit.DisplayFormat.FormatString);
                    break;
            }

            GroupSummary.Add(itemType, fieldName, gridColumn).DisplayFormat = dispFormat;
        }
        ...
        #region :: SetFixedColumn(+1 Overloading) :: Fixed Column을 설정합니다.
        ...
        /// 
        /// </remarks>
        public void SetFixedColumn(string columnName)
        {
            SetFixedColumn(columnName, FixedStyle.Left);
        }
        ...
        {
            //Color newColor = Color.FromArgb(OptionsSelection.MultiSelect ? 100 : 255, backColor.R, backColor.G, backColor.B);
        ...
        /// 
        /// </remarks>
        public void SetStyleFormatExpressionForeColor(string fieldName, Color foreColor, string expression, bool applyToRow = false)
        {
            GridColumn gridColumn = Columns[fieldName];
        ...
        /// 
        /// </remarks>
        public void SetStyleFormatExpressionBackColor(string fieldName, Color backColor, string expression, bool applyToRow = false)
        {
            GridColumn gridColumn = Columns[fieldName];
        ...
        /// <param name="unboundType"></param>
        public void SetUnboundExpression(string fieldName, string unboundExpression, UnboundType unboundType = UnboundType.Integer)
        {
            GridColumn gridColumn = Columns[fieldName];
        ...
        public void AcceptChanges()
        {
            ...
            if (NewRowEnableColumns == null) return;
        ...
        #region :: SetColumnAlign :: Grid Column 정렬을 설정합니다.

        /// <summary>
        /// Grid Column 정렬을 설정합니다.
        /// </summary>
        /// <param name="fieldName">Column Field 명</param>
        /// <param name="horzAlign">정렬</param>
        /// <remarks>
        /// 2016-05-17 최초생성 : 오인봉
        /// 변경내역
        /// 
        /// </remarks>
        public void SetColumnAlign(string fieldName, HorzAlign horzAlign)
        {
            GridColumn gridColumn = Columns[fieldName];
        ...
        {
            if (e.Button == MouseButtons.Left && IsMultiSelect && CalcHitInfo(new Point(e.X, e.Y)).HitTest == GridHitTest.ColumnButton)
                SelectAll();
        ...
        #region :: PGridView_CustomDrawColumnHeader :: Header 색을 변경합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void PGridView_CustomDrawColumnHeader(object sender, DevExpress.XtraGrid.Views.Grid.ColumnHeaderCustomDrawEventArgs e)
        {
            try
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void PGridView_CustomDrawFooter(object sender, DevExpress.XtraGrid.Views.Base.RowObjectCustomDrawEventArgs e)
        {
            try
            {
        ...
        private void PGridView_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                ...
                {
                    var val = dr[AppConfig.CHECKCOLUMNNAME];
        ...
        #region :: PGridView_PrintInitialize :: 프린트 기본값을 설정합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void PGridView_PrintInitialize(object sender, DevExpress.XtraGrid.Views.Base.PrintInitializeEventArgs e)
        {
            PrintingSystemBase pb = e.PrintingSystem as PrintingSystemBase;

            pb.PageSettings.Landscape = VisibleColumns.Count > 6 ? true : false;
            ...
            pb.Document.AutoFitToPagesWidth = VisibleColumns.Count < 9 ? 0 : 1;
            ...
            pb.Watermark.Font = new Font("나눔고딕", 36.25F);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void PGridView_CustomDrawRowIndicator(object sender, DevExpress.XtraGrid.Views.Grid.RowIndicatorCustomDrawEventArgs e)
        {
        ...
        #region :: PGridView_MouseWheel :: 마우스 휠이 작동하면 편집 중인 내용을 닫습니다.

        /// <summary>
        /// 마우스 휠이 작동하면 편집 중인 내용을 닫습니다.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        /// <exception cref="System.NotImplementedException"></exception>
        private void PGridView_MouseWheel(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            try
            {
                var view = sender as GridView;

                if (view.FocusedColumn.ColumnEdit == null) return;

                string editorTypeName = view.FocusedColumn.ColumnEdit.EditorTypeName;

                if (editorTypeName.In("SpinEdit")) return;
        ...
        #region :: RaisePopupMenuShowing :: 팝업메뉴에서 컬럼 사용자 지정 기능을 관리자만 가능하도록 합니다.

        /// <summary>
        /// 컬럼 사용자 지정을 관리자만 가능하도록 합니다.
        /// </summary>
        /// <param name="e"></param>
        protected override void RaisePopupMenuShowing(DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs e)
        {
            base.RaisePopupMenuShowing(e);

            //IUser uiForm = (GridControl.FindForm() as IUser);

            //if (uiForm == null || !e.HitInfo.InColumn) return;
            ...
            //    if (tag == "MenuColumnColumnCustomization" || tag == "MenuColumnRemoveColumn")
            //        item.Enabled = uiForm.CurrentUser.ISADMIN;
            //}
        }
        ...
        protected override void RaiseFocusedRowChanged(int prevFocused, int focusedRowHandle)
        {
            ...
            if (dr != null)
            {
                if (NewRowEnableColumns != null && NewRowEnableColumns.Length > 0)
                {
        ...
        protected override void RaiseValidateRow(DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            //if (cList.Count > 0)
            //{
            //    EBAP.Core.Localization.LanguageConverter locale = (GridControl.FindForm() as ILanguageConverter).LANGCONVERTER;
            //    //const string message = "필수입력 항목입니다. 값을 입력 하세요.\r\n작업을 취소하려면 [ESC]를 눌러주세요";
            //    string message = locale.GetLocaleMessage("IF_EssentialItemValidate");

            //    //foreach (GridColumn gc in cList)
            //    //{
            //    //    SetColumnError(gc, message, DevExpress.XtraEditors.DXErrorProvider.ErrorType.Information);
            //    //}
        ...
        public override void CopyToClipboard()
        {
            if (IsMultiSelect)
            {
                ...
                {
                    if ((GridControl as IExport).DisableExport && !(GridControl.FindForm() as IFrameUI).CurrentUser.ISADMIN) return;
