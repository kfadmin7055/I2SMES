...
{
    #region :: EBAP.UI.ADM.Management.UseLog ::
    ...
    ///               2. 
    /// History     :
    ///               1. (2018-04-10 오후 5:11:13 - 오인봉 - PC170245) : 최초 생성
    ///               2.
    ///               3.
    /// </remarks>
    public partial class UseLog : EBAP.Win.BaseFrame.UIFrame
    {
        ///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        #region :: UseLog_Load :: Form이 Load 시 발생합니다.
        ...
        private void InitGridControl()
        {
            ...
            viewList.InitColumn("PLANTCODE", "Plant", 140, 0, false, true);
            viewList.InitColumn("USERID", "User", 90, 0, false, true);
            viewList.InitColumn("USERNAME", "UserName", 80, 0, false, true);
            viewList.InitColumn("METHODNAME", "Method 명", 200, 0, false, true);
            viewList.InitColumn("QUERYID", "Query ID", 350, 0, false, true, DataType.MemoEx);
            viewList.InitColumn("STARTDTTM", "실행시작시간", 150, 0, false, true, DataType.DateTime, HorzAlign.Center);
            viewList.InitColumn("ENDDTTM", "실행종료시간", 150, 0, false, true, DataType.DateTime, HorzAlign.Center);
            //viewList.InitColumn("INTERVAL", "실행시간(ms)", 80, 0, false, true, DataType.Default, HorzAlign.Far);
            viewList.InitColumn("INTERVAL", "사용시간", 100, 0, 3, false, true, DataType.Number, HorzAlign.Far);
            ...
            pPivotGridControl1.InitColumn("PLANTCODE", "Plant", 75, PivotArea.FilterArea);
            pPivotGridControl1.InitColumn("USERNAME", "UserName", 75, PivotArea.FilterArea);
            pPivotGridControl1.InitColumn("DDATE", "Date", 100, PivotArea.FilterArea);
            pPivotGridControl1.InitColumn("CNT", "Count", 75, 0, PivotArea.DataArea, DataType.Number, DevExpress.Data.PivotGrid.PivotSummaryType.Sum);
        }
        ...
        private void InitControls()
        {
            ...
            deStartEnd.SetDate(date.AddDays(-13), date);

            chartList.InitSeriesTemplate("PLANTCODE", "DDATE", "CNT");
        }

        #endregion

        #region :: UseLog_Link :: Form 간 Data 전송 시 발생합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UseLog_Link(object sender, EventArgs e)
        {
            try
            {
                LinkData();
        ...
        #region :: UseLog_Selection :: MainForm의 조회 Button을 Click 하면 발생합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UseLog_Selection(object sender, EventArgs e)
        {
            try
            {
                SetFormLayout();
        ...
        {
            const string queryId = @"dbo.UseLog_Select";

            //간단한 조회일 경우
            DataSet ds;
            using (SqlBiz wb = new SqlBiz(AppConfig.WEBSERVICEURL))
            {
                //AsyncMethodCaller caller = new AsyncMethodCaller(wb.NTx_ExecuteDataSetFromQueryId);
                //IAsyncResult result = caller.BeginInvoke(queryId, DatabaseParams, null, null);
                //ds = caller.EndInvoke(result);

                //ds = wb.NTx_ExecuteFromQueryId(queryId, dbParams);
            ...
            DataSet ds2;
            const string queryId2 = @"dbo.ABC";

            string[] paramList = new string[] { "@STARTDTTM", "@ENDDTTM" };
            object[] valueList = new object[] { deStartEnd.StartDate, deStartEnd.EndDate };

            using (SqlBiz wb = new SqlBiz(AppConfig.WEBSERVICEURL))
            {
                ds2 = wb.NTx_ExecuteDataSet(ConnectionString.LOGDB, queryId2, AppConfig.COMMANDSP, paramList, valueList);
            }

            chartList.FillData(ds2);
            pPivotGridControl1.FillData(ds2);
        }
        ...
        #region :: UseLog_New :: MainForm의 신규 Button을 Click 하면 발생합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UseLog_New(object sender, EventArgs e)
        {
            try
            {
                NewData();
        ...
        #region :: UseLog_Save :: MainForm의 저장 Button을 Click 하면 발생합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UseLog_Save(object sender, EventArgs e)
        {
            try
            {
                SaveData();
        ...
        #region :: UseLog_Delete :: MainForm의 삭제 Button을 Click 하면 발생합니다.
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UseLog_Delete(object sender, EventArgs e)
        {
            try
            {
                DeleteData();
